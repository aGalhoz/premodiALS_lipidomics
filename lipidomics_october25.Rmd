...
Sex_age_all_participants = read_excel("data input/all_participants_ID_visits.xlsx") %>%
  select(PatientID, ParticipantCode, sex, age, Visit, type)
```
...
# IDs in CSF (131)
IDs_CSF <- read_delim("data input/export-2026-05-11-PREMODIALS-AKDTR_BRNO_CHUFR_HMCIL_HRO_KSSGCH_MRI_NIUSASSK_271participants/CSFAcquisition.csv",
                          delim = ";", escape_double = FALSE, trim_ws = TRUE) %>%
  filter(CsfSexists == 1) %>%
  group_by(PatientID) %>%
...
# Date info
date_CTR <- IDs_CSF_data %>%
  dplyr::left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "CTR")
date_ALS <- IDs_CSF_data %>%
  dplyr::left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "ALS")
date_mimic <- IDs_CSF_data %>%
  dplyr::left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "mimic")
date_PGMC <- IDs_CSF_data %>%
  dplyr::left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "PGMC")
date_SYMP <- IDs_CSF_data %>%
  dplyr::left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "SYMP")
date_NA <- IDs_CSF_data %>%
  dplyr::left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "NA")

skim(date_CTR %>% filter(Visit == "V0"))
skim(date_CTR %>% filter(Visit == "V1"))
skim(date_ALS  %>% filter(Visit == "V0"))
skim(date_ALS  %>% filter(Visit == "V1"))
skim(date_mimic %>% filter(Visit == "V0"))
skim(date_mimic %>% filter(Visit == "V1"))
skim(date_PGMC %>% filter(Visit == "V0"))
skim(date_PGMC %>% filter(Visit == "V1"))
skimr::skim(date_NA %>% filter(Visit == "V0"))
skimr::skim(date_NA %>% filter(Visit == "V1"))
skimr::skim(date_SYMP  %>% filter(Visit == "V0"))
skimr::skim(date_SYMP  %>% filter(Visit == "V1"))

date_all_CSF <- do.call("rbind",list(date_CTR,date_ALS,date_mimic,date_PGMC,date_SYMP,date_NA))

# -> Kruskal Wallis test between samples
kruskal_test(data = date_all_CSF %>% filter(Visit == "V0"),age~type)
dunn_test(data = date_all_CSF %>% filter(Visit == "V0"),age~type)
kruskal_test(data = date_all_CSF %>% filter(Visit == "V1"),age~type)
dunn_test(data = date_all_CSF %>% filter(Visit == "V1"),age~type)

## Sex analysis
Sex_CTR <- IDs_CSF_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "CTR")
Sex_ALS <-  IDs_CSF_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "ALS")
Sex_mimic <-  IDs_CSF_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "mimic")
Sex_PGMC <-  IDs_CSF_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "PGMC")
Sex_SYMP <-  IDs_CSF_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "SYMP")
Sex_NA =   IDs_CSF_data %>%
...
Sex_all_CSF <- do.call("rbind",list(Sex_ALS,
                                Sex_CTR,
                                Sex_mimic,
                                Sex_PGMC,
                                Sex_SYMP,
                                Sex_NA))
Sex_all_CSF <- Sex_all_CSF %>% filter(!is.na(sex))
Sex_all_CSF_V0 = Sex_all_CSF %>% filter(Visit == "V0")
Sex_all_CSF_V1 = Sex_all_CSF %>% filter(Visit == "V1")
table(Sex_all_CSF_V0$type,Sex_all_CSF_V0$sex)
fisher.test(Sex_all_CSF_V0$type,Sex_all_CSF_V0$sex)
pairwise_fisher_test(table(Sex_all_CSF_V0$type,Sex_all_CSF_V0$sex), p.adjust.method = "holm")
table(Sex_all_CSF_V1$type,Sex_all_CSF_V1$sex)
fisher.test(Sex_all_CSF_V1$type,Sex_all_CSF_V1$sex)
pairwise_fisher_test(table(Sex_all_CSF_V1$type,Sex_all_CSF_V1$sex), p.adjust.method = "holm")
```
...
# IDs in plasma (235)
IDs_plasma <- read_delim("data input/export-2026-05-11-PREMODIALS-AKDTR_BRNO_CHUFR_HMCIL_HRO_KSSGCH_MRI_NIUSASSK_271participants/BloodAcquisition.csv",
                          delim = ";", escape_double = FALSE, trim_ws = TRUE) %>%
  filter(BAexists == 1) %>% 
  group_by(PatientID) %>%
  mutate(Visit = paste0("V", row_number() - 1)) %>%
...
# Date info
date_CTR <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "CTR")
date_ALS <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "ALS")
date_mimic <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "mimic")
date_PGMC <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "PGMC")
date_SYMP <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "SYMP")
date_NA = IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "NA")
...
date_all_plasma <- do.call("rbind",list(date_CTR,date_ALS,date_mimic,date_PGMC,date_SYMP,date_NA))

# -> Kruskal Wallis test between samples
kruskal_test(data = date_all_plasma %>% filter(Visit == "V0"),age~type)
dunn_test(data = date_all_plasma %>% filter(Visit == "V0"),age~type)
kruskal_test(data = date_all_plasma %>% filter(Visit == "V1"),age~type)
dunn_test(data = date_all_plasma %>% filter(Visit == "V1"),age~type)

## Sex analysis
Sex_CTR <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "CTR")
Sex_ALS <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "ALS")
Sex_mimic <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "mimic")
Sex_PGMC <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "PGMC")
Sex_SYMP <- IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "SYMP")
Sex_NA = IDs_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "NA")

Sex_all_plasma <- do.call("rbind",list(Sex_ALS,
                                Sex_CTR,
                                Sex_mimic,
                                Sex_PGMC,
                                Sex_SYMP,
                                Sex_NA))
Sex_all_plasma <- Sex_all_plasma %>% filter(!is.na(sex))
Sex_all_plasma_V0 = Sex_all_plasma %>% filter(Visit == "V0")
Sex_all_plasma_V1 = Sex_all_plasma %>% filter(Visit == "V1")
table(Sex_all_plasma_V0$type,Sex_all_plasma_V0$sex)
fisher.test(Sex_all_plasma_V0$type,Sex_all_plasma_V0$sex)
pairwise_fisher_test(table(Sex_all_plasma_V0$type,Sex_all_plasma_V0$sex), p.adjust.method = "holm")
table(Sex_all_plasma_V1$type,Sex_all_plasma_V1$sex)
fisher.test(Sex_all_plasma_V1$type,Sex_all_plasma_V1$sex)
pairwise_fisher_test(table(Sex_all_plasma_V1$type,Sex_all_plasma_V1$sex), p.adjust.method = "holm")
...
# Date info
date_CTR <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "CTR")
date_ALS <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "ALS")
date_mimic <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "mimic")
date_PGMC <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "PGMC")
date_SYMP <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "SYMP")
date_NA = IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,age,Visit,type) %>%
  filter(type == "NA")
...
date_all_plasma_CSF <- do.call("rbind",list(date_CTR,date_ALS,date_mimic,date_PGMC))

# -> Kruskal Wallis test between samples
kruskal_test(data = date_all_plasma_CSF %>% filter(Visit == "V0"),age~type)
dunn_test(data = date_all_plasma_CSF %>% filter(Visit == "V0"),age~type)
kruskal_test(data = date_all_plasma_CSF %>% filter(Visit == "V1"),age~type)
dunn_test(data = date_all_plasma_CSF %>% filter(Visit == "V1"),age~type)

## Sex analysis
Sex_CTR <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "CTR")
Sex_ALS <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "ALS")
Sex_mimic <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "mimic")
Sex_PGMC <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "PGMC")
Sex_SYMP <- IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "SYMP")
Sex_NA = IDs_CSF_plasma_data %>%
  left_join(Sex_age_all_participants %>% dplyr::rename(Patient = ParticipantCode)) %>%
  select(PatientID,sex,Visit,type) %>%
  filter(type == "NA")

Sex_all_plasma_CSF <- do.call("rbind",list(Sex_ALS,
                                Sex_CTR,
                                Sex_mimic,
                                Sex_PGMC,
                                Sex_SYMP,
                                Sex_NA))
Sex_all_plasma_CSF <- Sex_all_plasma_CSF %>% filter(!is.na(sex))
Sex_all_plasma_CSF_V0 = Sex_all_plasma_CSF %>% filter(Visit == "V0")
Sex_all_plasma_CSF_V1 = Sex_all_plasma_CSF %>% filter(Visit == "V1")
table(Sex_all_plasma_CSF_V0$type,Sex_all_plasma_CSF_V0$sex)
fisher.test(Sex_all_plasma_CSF_V0$type,Sex_all_plasma_CSF_V0$sex)
pairwise_fisher_test(table(Sex_all_plasma_CSF_V0$type,Sex_all_plasma_CSF_V0$sex), p.adjust.method = "holm")
table(Sex_all_plasma_CSF_V1$type,Sex_all_plasma_CSF_V1$sex)
fisher.test(Sex_all_plasma_CSF_V1$type,Sex_all_plasma_CSF_V1$sex)
pairwise_fisher_test(table(Sex_all_plasma_CSF_V1$type,Sex_all_plasma_CSF_V1$sex), p.adjust.method = "holm")
...
run_DEx <- function(
  ...
  data_without_covariates = limma::removeBatchEffect(expr,)
...
coldata_CSF_new = coldata_CSF %>% 
  left_join(tube_batch_CSF %>%
              select(PatientID,Visit,Tube_number) %>%
              dplyr::rename(label = Tube_number)) %>%
  mutate(PatientID = ifelse(is.na(PatientID),"6H1HR8R7",PatientID)) %>%
  left_join(date_all_CSF %>% dplyr::select(PatientID,age,Visit)) %>%
  left_join(Sex_all_CSF %>% dplyr::select(PatientID,sex,Visit)) 
...
coldata_plasma_new = coldata_plasma %>% 
  left_join(tube_batch_plasma %>%
              select(PatientID,Visit,Tube_number) %>%
              dplyr::rename(label = Tube_number)) %>%
  mutate(PatientID = ifelse(is.na(PatientID),"6H1HR8R7",PatientID)) %>%
  left_join(date_all_plasma %>% dplyr::select(PatientID,age,Visit)) %>%
  left_join(Sex_all_plasma %>% dplyr::select(PatientID,sex,Visit)) 
...
significant_lipids_V1_ALS_CTR = res_plasma_V1$tables$`conditionALS-conditionCTR` %>% filter(adj.P.Val < 0.1) %>% rownames()
significant_lipids_V0_PGMC_CTR = res_plasma_V0$tables$`conditionPGMC-conditionCTR` %>% filter(adj.P.Val < 0.1) %>% rownames()
significant_lipids_V1_PGMC_CTR = res_plasma_V1$tables$`conditionPGMC-conditionCTR` %>% filter(adj.P.Val < 0.1) %>% rownames()

# check boxpots of lipids of interest
keep <- complete.cases(coldata_plasma_new[, c("batch", "sex", "age")])

plasma_data_nobatch <- removeBatchEffect(
  assay(lipidomics_data_plasma$se_filt_impute)[, keep],
  batch = coldata_plasma_new$batch[keep],
  batch2 = coldata_plasma_new$sex[keep],
  covariates = coldata_plasma_new$age[keep]
)
colnames(plasma_data_nobatch) = coldata_plasma_new[keep,]$ID


significant_lipids_ALS_CTR = significant_lipids_V1_ALS_CTR
significant_lipids_PGMC_CTR = c(significant_lipids_V0_PGMC_CTR,
                       significant_lipids_V1_PGMC_CTR) %>% unique()

# significant lipids/lipid family in ALS vs CTR
print(significant_lipids_ALS_CTR)
prefix_counts <- table(sub("^([A-Za-z]+).*", "\\1", significant_lipids_ALS_CTR))
print(sort(prefix_counts, decreasing = TRUE))

# significant lipids/lipid family in PGMC vs CTR
print(significant_lipids_PGMC_CTR)
prefix_counts <- table(sub("^([A-Za-z]+).*", "\\1", significant_lipids_PGMC_CTR))
print(sort(prefix_counts, decreasing = TRUE))

intersect(significant_lipids_ALS_CTR,
          significant_lipids_PGMC_CTR)

# merged
significant_both = c(significant_lipids_ALS_CTR,
                     significant_lipids_PGMC_CTR) %>% unique()
print(significant_both)
prefix_counts <- table(sub("^([A-Za-z]+).*", "\\1", significant_both))
print(sort(prefix_counts, decreasing = TRUE))

mat     <- plasma_data_nobatch        
coldata <- coldata_plasma_new[keep,] %>% left_join(PGMC_mutations_ID) 
 
id_col        <- "ID"
visit_col     <- "Visit"        
condition_col <- "condition"    
 
visit_levels     <- c("V0", "V1")
condition_levels <- c("ALS", "CTR", "mimic", "PGMC")
 
out_pdf   <- "plots/lipid_boxplots_ALS_CTR.pdf"

if (!is.null(id_col)) {
  rownames(coldata) <- coldata[[id_col]]
}
coldata <- coldata[colnames(mat), , drop = FALSE]
 
lipids_to_plot <- significant_lipids_ALS_CTR
 
pdf(out_pdf, width = 8, height = 5)
 
 
for (lip in lipids_to_plot) {
 
  df <- data.frame(
    value     = as.numeric(mat[lip, ]),
    Visit     = factor(coldata[[visit_col]], levels = visit_levels),
    Condition = factor(coldata[[condition_col]], levels = condition_levels),
    type      = factor(ifelse(is.na(coldata[["type"]]), "Non-PGMC",
                               as.character(coldata[["type"]])))) %>%
    filter(Condition %in% condition_levels)
  
   shape_values <- c(
    "Non-PGMC"    = 1,   # open circle, low-key
    "C9orf72"    = 16,  # filled circle
    "SOD1"    = 17,  # filled triangle
    "TARDBP"    = 15,  # filled square
    "FUS"    = 18,  # filled diamond
    "other"    = 8    # asterisk
  )
  ...
  p <- ggplot(df, aes(x = Condition, y = value, fill = Condition)) +
    ...
    geom_jitter(aes(shape = type),width = 0.15, size = 1, alpha = 0.6) +
...
out_pdf   <- "plots/lipid_boxplots_PGMC_CTR.pdf"
...
lipids_to_plot <- significant_lipids_PGMC_CTR
...
#### do all the same analyses but only with patients at both V0 and V1
# CSF at both V0 and V1
coldata_CSF_new_V0 = coldata_CSF_new %>% filter(Visit == "V0")
coldata_CSF_new_V1 = coldata_CSF_new %>% filter(Visit == "V1")
patientIDs_CSF_V0_V1 = intersect(coldata_CSF_new_V0$PatientID,
                                 coldata_CSF_new_V1$PatientID)
coldata_CSF_new_V0_V1 = coldata_CSF_new[which(coldata_CSF_new$PatientID %in% patientIDs_CSF_V0_V1),]
lipidomics_data_CSF_V0_V1 = lipidomics_data_CSF
lipidomics_data_CSF_V0_V1$se_filt_impute = lipidomics_data_CSF_V0_V1$se_filt_impute[,which(coldata_CSF_new$PatientID %in% patientIDs_CSF_V0_V1)]
colData(lipidomics_data_CSF_V0_V1$se_filt_impute) = S4Vectors::DataFrame(coldata_CSF_new_V0_V1)

# run for each visit
res_CSF_V0_IDs_V0_V1 <- run_DEx_visit(lipidomics_data_CSF_V0_V1$se_filt_impute, "V0")
res_CSF_V1_IDS_V0_V1 <- run_DEx_visit(lipidomics_data_CSF_V0_V1$se_filt_impute, "V1")

# summarise for each visit
res_CSF_V0_all_IDs_V0_V1 <- summarise_DE(res_CSF_V0_IDs_V0_V1, key = "name")
res_CSF_V1_all_IDs_V0_V1 <- summarise_DE(res_CSF_V1_IDS_V0_V1, key = "name")

writexl::write_xlsx(list(DE_CSF_V0 = res_CSF_V0_all_IDs_V0_V1, DE_CSF_V1 = res_CSF_V1_all_IDs_V0_V1),
                    "results/DE_CSF_by_visit_IDs_at_V0_V1.xlsx")

# - Plasma
coldata_plasma_new_V0 = coldata_plasma_new %>% filter(Visit == "V0")
coldata_plasma_new_V1 = coldata_plasma_new %>% filter(Visit == "V1")
patientIDs_plasma_V0_V1 = intersect(coldata_plasma_new_V0$PatientID,
                                 coldata_plasma_new_V1$PatientID)
coldata_plasma_new_V0_V1 = coldata_plasma_new[which(coldata_plasma_new$PatientID %in% patientIDs_plasma_V0_V1),]
lipidomics_data_plasma_V0_V1 = lipidomics_data_plasma
lipidomics_data_plasma_V0_V1$se_filt_impute = lipidomics_data_plasma_V0_V1$se_filt_impute[,which(coldata_plasma_new$PatientID %in% patientIDs_plasma_V0_V1)]
colData(lipidomics_data_plasma_V0_V1$se_filt_impute) = S4Vectors::DataFrame(coldata_plasma_new_V0_V1)

# run for each visit
res_plasma_V0_IDs_V0_V1 <- run_DEx_visit(lipidomics_data_plasma_V0_V1$se_filt_impute, "V0")
res_plasma_V1_IDs_V0_V1 <- run_DEx_visit(lipidomics_data_plasma_V0_V1$se_filt_impute, "V1")

# summarise for each visit
res_plasma_V0_all_IDs_V0_V1 <- summarise_DE(res_plasma_V0_IDs_V0_V1, key = "name")
res_plasma_V1_all_IDs_V0_V1 <- summarise_DE(res_plasma_V1_IDs_V0_V1, key = "name")

writexl::write_xlsx(list(DE_plasma_V0 = res_plasma_V0_all_IDs_V0_V1, DE_plasma_V1 = res_plasma_V1_all_IDs_V0_V1),
                    "results/DE_plasma_by_visit_IDs_at_V0_V1.xlsx")

# get significant lipids
significant_lipids_V1_ALS_CTR_V0_V1 = res_plasma_V1_IDs_V0_V1$tables$`conditionALS-conditionCTR` %>% filter(adj.P.Val < 0.1) %>% rownames()
significant_lipids_V0_PGMC_CTR_V0_V1 = res_plasma_V0_IDs_V0_V1$tables$`conditionPGMC-conditionCTR` %>% filter(adj.P.Val < 0.1) %>% rownames()
significant_lipids_V1_PGMC_CTR_V0_V1 = res_plasma_V1_IDs_V0_V1$tables$`conditionPGMC-conditionCTR` %>% filter(adj.P.Val < 0.1) %>% rownames()

# check boxpots of lipids of interest
keep <- complete.cases(coldata_plasma_new_V0_V1[, c("batch", "sex", "age")])

plasma_data_nobatch_V0_V1 <- removeBatchEffect(
  assay(lipidomics_data_plasma_V0_V1$se_filt_impute)[, keep],
  batch = coldata_plasma_new_V0_V1$batch[keep],
  batch2 = coldata_plasma_new_V0_V1$sex[keep],
  covariates = coldata_plasma_new_V0_V1$age[keep]
)
colnames(plasma_data_nobatch_V0_V1) = coldata_plasma_new_V0_V1[keep,]$ID


significant_lipids_ALS_CTR_V0_V1 = significant_lipids_V1_ALS_CTR_V0_V1
significant_lipids_PGMC_CTR_V0_V1 = c(significant_lipids_V0_PGMC_CTR_V0_V1,
                       significant_lipids_V1_PGMC_CTR_V0_V1)

# significant lipids/lipid family in ALS vs CTR
print(significant_lipids_ALS_CTR_V0_V1)
prefix_counts <- table(sub("^([A-Za-z]+).*", "\\1", significant_lipids_ALS_CTR_V0_V1))
print(sort(prefix_counts, decreasing = TRUE))

# significant lipids/lipid family in PGMC vs CTR
print(significant_lipids_PGMC_CTR_V0_V1)
prefix_counts <- table(sub("^([A-Za-z]+).*", "\\1", significant_lipids_ALS_CTR_V0_V1))
print(sort(prefix_counts, decreasing = TRUE))

intersect(significant_lipids_ALS_CTR_V0_V1,
          significant_lipids_PGMC_CTR_V0_V1)

mat     <- plasma_data_nobatch_V0_V1        
coldata <- coldata_plasma_new_V0_V1[keep,] %>% left_join(PGMC_mutations_ID) 
...
out_pdf   <- "plots/lipid_boxplots_ALS_CTR_V0_V1.pdf"
...
lipids_to_plot <- significant_lipids_ALS_CTR_V0_V1
...
out_pdf   <- "plots/lipid_boxplots_PGMC_CTR_V0_V1.pdf"
...
lipids_to_plot <- significant_lipids_PGMC_CTR_V0_V1
...
p
pdf("plots/DE_CSF_V0_PGMC_CTR_FDR10.pdf",width = 12,height = 9)
p
dev.off()
...
p
pdf("plots/DE_CSF_V1_PGMC_CTR_FDR10.pdf",width = 12,height = 9)
p
dev.off()
```
...
p = volcano_plot_contrast(res_plasma_V0_all,
                      ...
                      add_x      = 0.5, add_y = 0.1)
p
pdf("plots/DE_plasma_V0_ALS_CTR_FDR10.pdf",width = 12,height = 9)
p
dev.off()

# ALS vs CTR (both at V0 and V1)
p = volcano_plot_contrast(res_plasma_V0_all_IDs_V0_V1,
                      log2fc_col = "logFC_ALS_CTR",
                      padj_col   = "adj.P.Val_ALS_CTR",
                      group_pos  = "ALS",
                      group_neg  = "CTR",
                      title      = "ALS vs CTR at V0 - IDs at V0 and V1 (FDR = 10%)",
                      label_col = "name",
                      alpha      = 0.1,      
                      add_x      = 0.5, add_y = 0.1)
p
pdf("plots/DE_plasma_V0_ALS_CTR_FDR10_IDs_V0_V1.pdf",width = 12,height = 9)
p
dev.off()
...
p = volcano_plot_contrast(res_plasma_V1_all,
                      ...
                      add_x      = 0.7, add_y = 0.1)
p
pdf("plots/DE_plasma_V1_ALS_CTR_FDR5.pdf",width = 12,height = 9)
p
dev.off()
...
p = volcano_plot_contrast(res_plasma_V1_all,
                      ...
                      add_x      = 0.7, add_y = 0.1)
p
pdf("plots/DE_plasma_V1_ALS_CTR_FDR10.pdf",width = 12,height = 9)
p
dev.off()

p = volcano_plot_contrast(res_plasma_V1_all_IDs_V0_V1,
                      log2fc_col = "logFC_ALS_CTR",
                      padj_col   = "adj.P.Val_ALS_CTR",
                      group_pos  = "ALS",
                      group_neg  = "CTR",
                      title      = "ALS vs CTR at V1 - IDs at V0 and V1 (FDR = 10%)",
                      label_col = "name",
                      alpha      = 0.1,      
                      add_x      = 0.7, add_y = 0.1)
p
pdf("plots/DE_plasma_V1_ALS_CTR_FDR10_IDs_V0_V1.pdf",width = 12,height = 9)
p
dev.off()
...
p = volcano_plot_contrast(res_plasma_V0_all,
                      ...
                      add_x      = 0.7, add_y = 0.1)
p
pdf("plots/DE_plasma_V0_PGMC_CTR_FDR5.pdf",width = 12,height = 9)
p
dev.off()
...
p = volcano_plot_contrast(res_plasma_V0_all,
                      ...
                      add_x      = 0.7, add_y = 0.1)
p
pdf("plots/DE_plasma_V0_PGMC_CTR_FDR10.pdf",width = 12,height = 9)
p
dev.off()
...
p = volcano_plot_contrast(res_plasma_V0_all_IDs_V0_V1,
                      ...
                      group_pos  = "PGMC",
                      group_neg  = "CTR",
                      title      = "PGMC vs CTR at V0 - IDs at V0 and V1 (FDR = 10%)",
                      label_col = "name",
                      alpha      = 0.1,      
                      add_x      = 0.7, add_y = 0.1)
p
pdf("plots/DE_plasma_V0_PGMC_CTR_FDR10_IDs_V0_V1.pdf",width = 12,height = 9)
p
dev.off()
...
p = volcano_plot_contrast(res_plasma_V1_all,
                      ...
                      add_x      = 0.9, add_y = -0.1)
p
pdf("plots/DE_plasma_V1_PGMC_CTR_FDR10.pdf",width = 12,height = 9)
p
dev.off()
...
p = volcano_plot_contrast(res_plasma_V1_all_IDs_V0_V1,
                      ...
                      group_pos  = "PGMC",
                      group_neg  = "CTR",
                      title      = "PGMC vs CTR at V1 - IDs at V0 and V1 (FDR = 10%)",
                      label_col = "name",
                      alpha      = 0.1,      
                      add_x      = 0.9, add_y = -0.1)
p
pdf("plots/DE_plasma_V1_PGMC_CTR_FDR10_IDs_V0_V1.pdf",width = 12,height = 9)
p
dev.off()
