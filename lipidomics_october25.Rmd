...
volcano_plot_contrast <- function(
  ...
  # create DE column if not present: group_pos for logFC>0, group_neg for logFC<0, 'ns' for NA/0
...
res_CSF = run_DEx(
    lipidomics_data_CSF$se_filt_impute,
    covariates = c("sex","batch","age"),
    contrasts = c("PGMC_vs_CTR" = "PGMC-CTR",
                  "ALS_vs_CTR"="ALS-CTR",
...
res_CSF_ALS_CTR = res_CSF$tables$`conditionALS-conditionCTR` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_ALS_CTR = logFC,
         P.Value_ALS_CTR = P.Value,
         adj.P.Val_ALS_CTR = adj.P.Val)
res_CSF_PGMC_CTR = res_CSF$tables$`conditionPGMC-conditionCTR` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_PGMC_CTR = logFC,
         P.Value_PGMC_CTR = P.Value,
         adj.P.Val_PGMC_CTR = adj.P.Val)
res_CSF_mimic_CTR = res_CSF$tables$`conditionmimic-conditionCTR` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val)  %>%
  dplyr::rename(logFC_mimic_CTR = logFC,
         P.Value_mimic_CTR = P.Value,
         adj.P.Val_mimic_CTR = adj.P.Val)
res_CSF_ALS_PGMC = res_CSF$tables$`conditionALS-conditionPGMC` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val)  %>%
  dplyr::rename(logFC_ALS_PGMC = logFC,
         P.Value_ALS_PGMC = P.Value,
         adj.P.Val_ALS_PGMC = adj.P.Val)
res_CSF_ALS_mimic = res_CSF$tables$`conditionALS-conditionmimic` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_ALS_mimic = logFC,
         P.Value_ALS_mimic = P.Value,
         adj.P.Val_ALS_mimic = adj.P.Val)
res_CSF_mimic_PGMC = res_CSF$tables$`conditionmimic-conditionPGMC` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_mimic_PGMC = logFC,
         P.Value_mimic_PGMC = P.Value,
         adj.P.Val_mimic_PGMC = adj.P.Val)

res_CSF_all = Reduce(function(x, y) merge(x, y, by = "name", all = TRUE),
                      list(res_CSF_ALS_CTR,
                        res_CSF_PGMC_CTR,
                        res_CSF_mimic_CTR,
                        res_CSF_ALS_PGMC,
                        res_CSF_ALS_mimic,
                        res_CSF_mimic_PGMC))
writexl::write_xlsx(res_CSF_all,"results/DE_CSF.xlsx")
...
res_plasma = run_DEx(
    lipidomics_data_plasma$se_filt_impute,
    covariates = c("sex","batch","age"),
    contrasts = c("PGMC_vs_CTR" = "PGMC-CTR",
                  "ALS_vs_CTR"="ALS-CTR",
                  "mimic_vs_CTR" = "mimic-CTR",
...
res_plasma_ALS_CTR = res_plasma$tables$`conditionALS-conditionCTR` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_ALS_CTR = logFC,
         P.Value_ALS_CTR = P.Value,
         adj.P.Val_ALS_CTR = adj.P.Val)
res_plasma_PGMC_CTR = res_plasma$tables$`conditionPGMC-conditionCTR` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_PGMC_CTR = logFC,
         P.Value_PGMC_CTR = P.Value,
         adj.P.Val_PGMC_CTR = adj.P.Val)
res_plasma_mimic_CTR = res_plasma$tables$`conditionmimic-conditionCTR` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val)  %>%
  dplyr::rename(logFC_mimic_CTR = logFC,
         P.Value_mimic_CTR = P.Value,
         adj.P.Val_mimic_CTR = adj.P.Val)
res_plasma_ALS_PGMC = res_plasma$tables$`conditionALS-conditionPGMC` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val)  %>%
  dplyr::rename(logFC_ALS_PGMC = logFC,
         P.Value_ALS_PGMC = P.Value,
         adj.P.Val_ALS_PGMC = adj.P.Val)
res_plasma_ALS_mimic = res_plasma$tables$`conditionALS-conditionmimic` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_ALS_mimic = logFC,
         P.Value_ALS_mimic = P.Value,
         adj.P.Val_ALS_mimic = adj.P.Val)
res_plasma_mimic_PGMC = res_plasma$tables$`conditionmimic-conditionPGMC` %>% 
  dplyr::select(name,logFC,P.Value,adj.P.Val) %>%
  dplyr::rename(logFC_mimic_PGMC = logFC,
         P.Value_mimic_PGMC = P.Value,
         adj.P.Val_mimic_PGMC = adj.P.Val)

res_plasma_all = Reduce(function(x, y) merge(x, y, by = "name", all = TRUE),
                      list(res_plasma_ALS_CTR,
                        res_plasma_PGMC_CTR,
                        res_plasma_mimic_CTR,
                        res_plasma_ALS_PGMC,
                        res_plasma_ALS_mimic,
                        res_plasma_mimic_PGMC))
writexl::write_xlsx(res_plasma_all,"results/DE_plasma.xlsx")
